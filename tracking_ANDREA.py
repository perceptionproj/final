...
for i in range(n_images):
	...
	features_curr = cv2.goodFeaturesToTrack(frame_gray_curr, maxCorners=of_max_objs, qualityLevel=of_quality, minDistance=of_min_dist, mask=mask_roi)
	...
	if (i>0):
		...
		if (features_flow.shape[0] >= min_N):
			...
			# make sure that the x coordinate of the center of the object is in the region of the conveyor:
			if (features_flow_center[0]<=conveyor_x1 and features_flow_center[0]>=conveyor_x0):
				...
				cv2.circle(frame_rgb_curr, roi_hist_center, 4, (0, 255, 0), -1)
				# draw points that passed all the tests
				for i in range(features_flow.shape[0]):			
					cv2.circle(frame_rgb_curr, (features_flow[i][0][0], features_flow[i][0][1]), 1, (0, 255, 0), -1)
